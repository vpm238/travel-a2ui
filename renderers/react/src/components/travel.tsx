...
/**
 * A stable number from a string, for choosing artwork that never changes.
 *
 * The same property has to get the same picture on every render, in every
 * client, forever — a card whose image shuffles when the list re-sorts reads as
 * broken. FNV-1a because it is four lines and good enough to spread names
 * across a handful of buckets.
 */
...
/**
 * The picture on a property card, when there is no photograph.
 *
 * There are no photographs — the stays are invented, and a stock image of a
 * hotel that does not exist would be the one dishonest pixel in the whole
 * app. What was here instead was a bed glyph centred on a grey rectangle,
 * which reads as a *missing* photo: the card looks broken rather than
 * illustrated, and that is most of why these cards looked fake.
 *
 * So: a drawing, generated from the name. A skyline whose heights, windows and
 * sky come out of `seedOf`, so every property has its own and keeps it. It does
 * not pretend to be a photograph, which means it cannot be mistaken for one.
 */
function Skyline({ name }: { name: string }) {
  const seed = seedOf(name);
  const hue = seed % 360;
  // Eight towers across the full width, each with a height and a window grid,
  // all from the seed.
  const towers = Array.from({ length: 8 }, (_, index) => {
    const local = seedOf(`${name}:${index}`);
    return {
      x: index * 17 + 1,
      width: 12 + (local % 4),
      height: 18 + (local % 40),
  ...
  return (
    <div className="tv-art" style={{ ['--art-hue' as string]: String(hue) }}>
      {/* The sky is the container's own background, so it fills a card of any
          height without the drawing being stretched or cropped to do it. Only
          the skyline is drawn, and it sits on the bottom edge. */}
      <svg viewBox="0 0 138 62" preserveAspectRatio="xMidYMax meet" aria-hidden>
        <circle cx={112 - (seed % 70)} cy="13" r="7" fill="hsl(46 96% 76%)" opacity="0.92" />
        {towers.map((tower) => (
          <g key={tower.x}>
            <rect
              x={tower.x}
              ...
              width={tower.width}
              height={tower.height}
              fill={`hsl(${hue} 30% ${20 + (tower.x % 10)}%)`}
              opacity="0.94"
            />
            {Array.from({ length: tower.windows }, (_, row) => (
              <rect
                key={row}
                x={tower.x + 3}
                y={62 - tower.height + 7 + row * 9}
                width={tower.width - 6}
                height="4"
                rx="1"
                fill="hsl(46 96% 84%)"
                opacity={(tower.x + row) % 3 === 0 ? 0.9 : 0.4}
              />
            ))}
          </g>
        ))}
...
/**
 * A composed price, split back into its two halves for the eye.
 *
 * The server sends one string — "$352 each · $704 for 2" — because that is what
 * binds to a card's one `price` property and what a client with no opinion
 * should show. A client *with* an opinion can do better: the number they will
 * pay reads large, and what it is made of reads small underneath. Splitting on
 * the separator the server composed with keeps that entirely presentational —
 * nothing is recomputed here, and a price with no separator just renders whole.
 */
  ...
  return (
    ...
    >
      ...
      <div className="tv-flight__price">
        ...
        <strong>
          {isPending(node['price'], scope) ? (
            <span className="tv-pending" style={{ width: 62 }} aria-hidden />
          ) : (
            fare
          )}
        </strong>
...
export function HotelCard({ node, scope, ctx }: ComponentProps) {
  ...
  const rating = resolveText(node['rating'], scope);
  const neighborhood = resolveText(node['neighborhood'], scope);
  const [nightly, stay] = splitPrice(resolveText(node['price'], scope));
  ...
  return (
    ...
    >
      <div className="tv-hotel__media">
        {isSafeUrl(image) ? <img src={image} alt="" loading="lazy" /> : <Skyline name={name} />}
        {badge ? <span className="tv-badge tv-badge--onMedia">{badge}</span> : null}
      </div>
      <div className="tv-hotel__body">
        <div className="tv-hotel__head">
          <h4>{name}</h4>
          {rating ? <span className="tv-hotel__score">{rating}</span> : null}
        </div>
        {neighborhood ? (
          <p className="tv-hotel__where">
            <Icon name="pin" />
            {neighborhood}
          </p>
        ) : null}
